...
class CustomerOnboarding(Document):
	def before_submit(self):
		...
		job = frappe.enqueue(
			"sccc.sccc.doctype.customer_onboarding.customer_onboarding.provision_site",
			full_name=self.full_name,
			email=self.email,
			site_name=self.site_name,
			modules=self.modules,
			onboarding_doc=self.name,
		...
		if job:
			...
			self.db_set("provision_result", f"RQ Job: {job_id}")
			frappe.msgprint(
				f"Site creation has started in the background.<br>"
				f"You can track the job here: <a href='/app/rq-job'>RQ Jobs</a>"
			)
	...
	try:
		...
		if job_id:
			frappe.db.set_value(
				"Customer Onboarding",
				onboarding_doc,
				{"status": "Provisioning", "provision_result": f"Bench job id: {job_id}"},
			)
	except Exception:
		frappe.db.set_value(
			"Customer Onboarding",
			onboarding_doc,
			{"status": "Failed", "provision_result": frappe.get_traceback()},
		)
		raise
...
def create_site(site_name, mysql_password, admin_password, key, full_name, email, onboarding_doc, a_async=True):
	...
	job = frappe.enqueue(
		"sccc.sccc.doctype.customer_onboarding.customer_onboarding.job_site_creation",
		commands=commands,
...
def job_site_creation(commands, doctype, key, site_name, onboarding_doc, full_name, email, *args, **kwargs):
	try:
		...
		commands_to_run = [
			...
			f"bench --site {site_name} execute sccc.doctype.customer_onboarding.customer_onboarding.run_setup_wizard --kwargs '{kwargs_json}'",
			f"bench --site {site_name} execute sccc.doctype.customer_onboarding.customer_onboarding.create_or_update_user --kwargs '{kwargs_json}'"
		]
		...
		frappe.db.set_value(
			"Customer Onboarding",
			onboarding_doc,
			{"status": "Provisioned", "provision_result": "Site created and setup wizard started"},
		)
	except Exception:
		frappe.db.set_value(
			"Customer Onboarding",
...
def _update_master_site(onboarding_doc, values: dict):
	...
	frappe.connect()
	frappe.db.set_value("Customer Onboarding", onboarding_doc, values)
	frappe.db.commit()
